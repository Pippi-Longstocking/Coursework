...
//grid[row].length = the number of columns
...
public class Maze implements Drawable, Runnable
{
   ...
   { 
      ...
      if ( (row <= grid.length - 1) && (row >= 0) && (column <= grid[row].length - 1) && (column >= 0))
      {
         ...
         if ( grid[row][column] == SPACE )
         {
            ...
            else
            {
               ...
               done =   traverse(row, column - 1);  //up
            	
               if (!done)
               {
                  done =  traverse(row - 1, column); //left    
                 }
               if (!done)
               {
                  done =   traverse(row, column + 1);    //down
               }
               if (!done)
               {
                  done =   traverse(row + 1, column); //right
               }
